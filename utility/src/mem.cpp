...
#else
  snprintf(_name, NAME_MAX, "/tmp/%s", memname);
  // srand(time(0));
  key_t key = ftok(_name, 'W'); // rand() % 125 + 1);
  _shfd = shmget(key, _size, 0666);
  if (_shfd == -1)
    return MDERR_MEMMAPPING_SHMOPEN;
  if ((_dataptr = shmat(_shfd, NULL, 0)) == MAP_FAILED)
    return MDERR_MEMMAPPING_FAILED;
#endif // USING_MMAP
 is_writble_ = (false);
  return (0);
...
int MemMapping::OpenWrite(const char *filepath, const char *memname,
  ...
  {
    FILE *fp = 0;
    if (fopen_s(&fp, _name, "w+b"))
      return -1;
    fclose(fp);
  }
  srand(time(0));
  _key = ftok(_name, 'W'); // rand() % 125 + 1);
  _shfd = shmget(_key, _size, IPC_CREAT | 0666);
  if (_shfd == -1) {
    printf("error is %d, %s, %lu, %d\n", errno, strerror(errno), _size, _key);
    return MDERR_MEMMAPPING_SHMOPEN;
  }
