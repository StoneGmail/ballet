#ifndef CTPIF_MATLAB_H
#define CTPIF_MATLAB_H
 ...
 */
extern CONFIGPTR mexInitConfig(const char *filepath);
/**
 * @brief releaseCtpMdIf Release the Ctp Interface for market data
 */
extern void mexReleaseConfig(CONFIGPTR param);

extern void mexConnectToCtpTdif(INTERFACE interface);

extern INTERFACE mexGetCtpMdif(CONFIGPTR param);

extern void mexReleaseCtpMdif(INTERFACE mdif);

extern INTERFACE mexGetCtpTdif(CONFIGPTR param, const char *logpath);

extern void mexReleaseCtpTdif(INTERFACE tdif);

extern void mexConnectToCtpMdif(INTERFACE interface);


extern int mexStop(INTERFACE tdif);
extern int mexLogin(INTERFACE tdif);
extern int mexLogout(INTERFACE tdif);

extern int mexQueryInstrument(INTERFACE tdif, const char *instrument = NULL);
extern int mexQueryAccount(INTERFACE tdif);
extern int mexQueryInvestorPosition(INTERFACE tdif, const char *instrument = NULL);
extern int mexQueryOrder(INTERFACE tdif, const char *instrument = NULL, const char *exchange = NULL,
                                 const char *ordersysid = NULL, const char *begintime = NULL,
                                 const char *endtime = NULL);
extern int mexQueryTrade(INTERFACE tdif, const char *instrument = NULL, const char *exchange = NULL,
                                 const char *tradeid = NULL, const char *begintime = NULL,
                                 const char *endtime = NULL);

extern int mexOrderInsertLimitPrice(INTERFACE tdif, const char *instrument, char cDirection,
...
extern int mexOrderInsertLimitd(INTERFACE tdif, const char *instrument, char cDirection,
                                         double fPrice, int nVolume);
extern int mexOrderInsertCondtn(INTERFACE tdif, const char *instrument, char cDirection,
                                         double fPrice, int nVolume, double fConditionPrice,
                                         char cTrigerCondition, char cPriceCondition);
extern int mexOrderCorverCondtn(INTERFACE tdif, const char *instrument, char cDirection,
                                         double fPrice, int nVolume, double fConditionPrice,
                                         char cTrigerCondition, char cPriceCondition);
extern int mexOrderInsertParked(INTERFACE tdif);
extern int mexOrderCorver(INTERFACE tdif, const char *instrument, char cDirection, double fPrice,
                                  int nVolume);
extern int mexOrderRecall(INTERFACE tdif, Order *order);

extern char *mexGetOrderStatus(INTERFACE tdif, char status, char *buf, int len);

#ifdef __cplusplus
}
#endif

#endif // CTPIF_MATLAB_H
